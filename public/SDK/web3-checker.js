(async () => {
  ...
  const contractAddress = "0x2a0314c30bb89d2a1b743f3f6ec3f3c418872aa0";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  connectBtn.addEventListener("click", async () => {
    ...
    try {
      const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
      const wallet = accounts[0];
      statusText.innerText = `✅ Wallet connected: ${wallet}`;
      ...
      if (parseInt(ownsToken) > 0) {
        ...
        if (originalPriceElement && discountedPriceElement) {
          const originalPrice = parseFloat(originalPriceElement.innerText.replace(/[^\d\.]/g, ''));
          if (!isNaN(originalPrice)) {
            const discounted = (originalPrice * 0.85).toFixed(2);
            discountedPriceElement.innerText = '€' + discounted;
            discountedPriceElement.style.display = 'inline';
          }
        }
      } else {
        console.log("❌ No student token found.");
        statusText.innerText = "❌ No student token found.";
      }
    } catch (err) {
